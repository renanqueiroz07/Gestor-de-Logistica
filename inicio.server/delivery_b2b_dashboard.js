...
function updateRouteDetails(route = selectedRoute) {
  const routeDetailsMeta = document.getElementById('routeDetailsMeta');
  const routeDetailsStatus = document.getElementById('routeDetailsStatus');
  if (routeDetailsMeta) {
    routeDetailsMeta.textContent = route === 'Todas'
      ? 'Todas as rotas · Hub central · Moto 🛵'
      : `${route} · Hub central · Moto 🛵`;
  }
  if (routeDetailsStatus) {
    routeDetailsStatus.textContent = route === 'Todas' ? '● Atualizado agora' : `● ${route} selecionada`;
  }
}

function setActiveRoute(route) {
  selectedRoute = route;
  document.querySelectorAll('.route-toggle-btn').forEach(button => {
    button.classList.toggle('active', button.dataset.route === route);
  });
  document.querySelectorAll('.route-card-btn').forEach(button => {
    button.classList.toggle('active', button.dataset.route === route);
  });
  showRoute(route);
  loadOrdersOnMap(route);
...
function loadOrdersOnMap(route = selectedRoute) {
  // Limpar marcadores existentes
  markers.forEach(marker => map.removeLayer(marker));
  markers = [];

  const filteredOrders = route === 'Todas'
    ? ordersData
    : ordersData.filter(order => order.route === route);
...
function acceptOrder() {
  ...
  addOrderToList({
    ...
    time: 'agora',
    route: selectedRoute && selectedRoute !== 'Todas' ? selectedRoute : 'Rota A'
  });
...
const ordersData = [
  { num:'#4721', client:'Pizzaria Roma', addr:'Rua Oscar Freire, 254', status:'transit', statusLabel:'Em trânsito', value:'R$1.240', time:'10min', route:'Rota A' },
  { num:'#4720', client:'Sushi Bar Yamamoto', addr:'Alameda Santos, 811', status:'transit', statusLabel:'Em trânsito', value:'R$890', time:'22min', route:'Rota A' },
  { num:'#4719', client:'Café Blend', addr:'Rua Bela Cintra, 306', status:'route', statusLabel:'Roteado', value:'R$340', time:'35min', route:'Rota A' },
  { num:'#4718', client:'Burguer Factory', addr:'Av. Rebouças, 1400', status:'transit', statusLabel:'Em trânsito', value:'R$720', time:'40min', route:'Rota B' },
  { num:'#4717', client:'Deli Central', addr:'Rua da Consolação, 88', status:'pending', statusLabel:'Pendente', value:'R$290', time:'45min', route:'Rota B' },
  { num:'#4716', client:'Restaurante Bella', addr:'Rua Haddock Lobo, 620', status:'done', statusLabel:'Entregue', value:'R$1.100', time:'1h', route:'Rota B' },
  { num:'#4715', client:'Temaki House', addr:'Av. Paulista, 327', status:'done', statusLabel:'Entregue', value:'R$560', time:'2h', route:'Rota C' },
];
...
function updateRouteStops(route = selectedRoute) {
  const routeOrders = ordersData.filter(o => (
    (route === 'Todas' || o.route === route) &&
    (o.status === 'route' || o.status === 'transit' || o.status === 'pending')
  ));
  const stops = [
  ...
  if (!stopsContainer) {
    if (previewText) previewText.textContent = `Rota com ${routeOrders.length} parada(s) gerada automaticamente para o próximo despacho.`;
    return;
  }
...
function addOrder() {
  const clientInput = document.getElementById('fi-client');
  const addrInput = document.getElementById('fi-addr');
  const itemsInput = document.getElementById('fi-items');
  const valueInput = document.getElementById('fi-value');
  const client = clientInput?.value || 'Novo Cliente';
  const addr = addrInput?.value || 'Endereço não informado';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (mapEl && typeof L !== 'undefined') {
    ...
    updateRouteStops(selectedRoute);
  }
});
